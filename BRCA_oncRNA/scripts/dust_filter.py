...
project_dir = "/rumi/shams/jwang/BRCA_oncRNA"
tcga_metadata = pd.read_csv(f"{project_dir}/metadata/TCGA_metadata.csv").set_index("UUID")
...
tcga_files = [f.path for f in os.scandir("/rumi/shams/jwang/TCGA") if f.is_dir() and f.name in tcga_metadata.index]
...
def processInput(f):
    ...
    out = f"{project_dir}/data/TCGA/{study_code}.dust.bam"
